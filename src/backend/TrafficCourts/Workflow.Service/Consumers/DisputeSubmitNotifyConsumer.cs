        ...
        public async Task Consume(ConsumeContext<SubmitNoticeOfDispute> context)
        {
            ...
            if (template is not null)
            {
                ...
                var emailMessage = new SendEmail()
                {
                    ...
                    Subject = template.SubjectTemplate.Replace("<ticketid>", dispute.ViolationTicket.TicketNumber),
                    PlainTextContent = template.PlainContentTemplate?.Replace("<ticketid>", dispute.ViolationTicket.TicketNumber)
                };
