...
public class DomainModelMappingTestGenerator
{
    ...
    {
        var types = typeof(Domain.Models.Dispute).Assembly.GetTypes().Where(IsTargetClass).OrderBy(_ => _.Name);

        foreach (var type in types)
        {
            string name = type.Name;
            _output.WriteLine("/// <summary>");
            _output.WriteLine($"/// Tests to ensure mapping from <see cref=\"TrafficCourts.Common.OpenAPIs.OracleDataApi.v1_0.{name}\"/>");
            _output.WriteLine($"/// to <see cref=\"TrafficCourts.Domain.Models.{name}\"/> and back again create the same object");
            _output.WriteLine("/// </summary>");
            _output.WriteLine($"public class {name}_MappingTest : RoundTripMappingTest<TrafficCourts.Common.OpenAPIs.OracleDataApi.v1_0.{name}, TrafficCourts.Domain.Models.{name}>");
            _output.WriteLine("{");
            _output.WriteLine($"    public {name}_MappingTest(ITestOutputHelper output) : base(output)");
            _output.WriteLine("    {");
            _output.WriteLine("    }");
            _output.WriteLine("}");
            _output.WriteLine("");
    ...
    private static bool IsTargetClass(Type type) => type.IsClass && IncludeType(type) && type != typeof(PagedCollection<>);
